"""OpenCode real-client certification harness."""
...
from mcp_multiplex.adapters import parse_opencode_config
...
from mcp_multiplex.install import install_opencode_control_plane
...
@dataclass(frozen=True)
class OpenCodeCertificationResult:
    ok: bool
    work_dir: str
    ...
    def transcript(self) -> str:
        lines = [
            ...
            f"OpenCode version: `{self.opencode_version}`",
            f"Plan: `{self.plan_id}`",
        ...
        lines.extend(
            [
                ...
                "```",
                "",
                "## Project Config After Apply",
                "",
                "```json",
                self.config_after.rstrip(),
                "```",
                "",
                "## Project Config After Rollback",
                "",
                "```json",
                ...
                "## Review Gate",
                "",
                "Final human approval is still required before upgrading OpenCode to certified.",
            ]
        )
        return "\n".join(lines) + "\n"


def run_opencode_certification(
    *,
    ...
    if port != 30000:
        raise CertificationError("OpenCode certification must use Hub data-plane port 30000")
    resolved_opencode = shutil.which(opencode_bin)
    if resolved_opencode is None:
        raise CertificationError(f"OpenCode CLI not found: {opencode_bin}")
    ...
    global_config_path = global_dir / "opencode.jsonc"
    project_config_path = project / "opencode.jsonc"
    connection = connect(root / "multiplex.db")
    steps: list[CertificationStep] = []

    config_before = _direct_project_opencode_config()
    project_config_path.write_text(config_before, encoding="utf-8")
    control_install = install_opencode_control_plane(
        connection,
        home=home,
        config_path=global_config_path,
        backup_dir=root / "install-backups",
        actor="opencode_certification",
    )
    if control_install.token is None:
        raise CertificationError("OpenCode control-plane installer did not issue an auth token")
    control_token = control_install.token
    global_config = global_config_path.read_text(encoding="utf-8")
    ...
    steps.append(
        CertificationStep(
            ...
            True,
            (
                "installed authenticated mcp_hub with the OpenCode installer "
                "and project direct context7 fixture"
            ),
            {
    ...
    parsed_before = parse_opencode_config(project_config_path, agent_id=OPENCODE_AGENT_ID)
    ...
    steps.append(
        CertificationStep(
            "detect_drift",
            True,
            "OpenCode project direct context7 was detected as an active direct bypass",
    ...
    approval = ApprovalStore(connection).approve(
        ...
        comment="TASK-042 disposable certification approval",
    )
    ...
    config_after = project_config_path.read_text(encoding="utf-8")
    rewritten = json.loads(config_after)
    if rewritten.get("mcp", {}).get("context7") != {
        "enabled": True,
        "type": "remote",
        "url": f"{HUB_BASE_URL}/servers/context7/mcp",
    }:
        raise CertificationError("OpenCode project config was not rewritten to Hub remote URL")
    steps.append(
        CertificationStep(
            "rewrite_through_hub",
            True,
            "approved plan rewrote project context7 through the Hub and preserved enabled state",
    ...
    try:
        opencode_version = _run_opencode(
            [resolved_opencode, "--version"],
            home=home,
            control_token=control_token.token,
        ).strip()
        opencode_list = _run_opencode(
            [resolved_opencode, "mcp", "list"],
            home=home,
            xdg_config=xdg_config,
            cwd=project,
            control_token=control_token.token,
        )
        if f"{HUB_BASE_URL}/servers/context7/mcp" not in opencode_list:
            raise CertificationError("OpenCode did not list context7 as Hub-routed")
        if f"{HUB_BASE_URL}/servers/mcp_hub/mcp" not in opencode_list:
            raise CertificationError("OpenCode did not list global mcp_hub as Hub-routed")
        steps.append(
            CertificationStep(
                "verify_opencode_merge_and_hub_routing",
                True,
                "real OpenCode CLI merged global mcp_hub with project context7 Hub routing",
                {"output": opencode_list},
            )
        )
    ...
    config_after_rollback = project_config_path.read_text(encoding="utf-8")
    if config_after_rollback != config_before:
        raise CertificationError("rollback did not restore exact OpenCode project config bytes")
    steps.append(
        CertificationStep(
            "rollback",
            True,
            "rollback restored exact project pre-image bytes",
            {"restored_hash": rollback.restored_hash},
        )
    )
    return OpenCodeCertificationResult(
        ok=True,
        work_dir=str(root),
...
def _work_dir(work_dir: Path | None) -> Path:
    ...
    return Path(tempfile.mkdtemp(prefix="mcp-multiplex-opencode-cert-")).resolve()


def _direct_project_opencode_config() -> str:
    payload = {
        "mcp": {
            "context7": {
                "type": "local",
                "command": [sys.executable, str(_fake_backend_path())],
                "enabled": True,
            }
        }
    }
    ...
    *,
    ...
    if result.returncode != 0:
        raise CertificationError(
            f"OpenCode command failed ({result.returncode}): {' '.join(command)}\n"
...
__all__ = ["OpenCodeCertificationResult", "run_opencode_certification"]
